...
module.exports = class Dock {
  ...
  render(state) {
    if (this.element == null) {
      ...
      this.cursorOverlayElement.classList.add(
    ...
    const size = Math.max(
      ...
      state.size ||
        (state.draggingItem &&
          getPreferredSize(state.draggingItem, this.location)) ||
        DEFAULT_INITIAL_SIZE
    );

    // We need to change the size of the mask...
    this.maskElement.style[this.widthOrHeight] = `${
      shouldBeVisible ? size : 0
    }px`;
    ...
    this.toggleButton.update({
      ...
      visible:
        ...
        (!shouldBeVisible &&
          state.draggingItem &&
          isItemAllowed(state.draggingItem, this.location)),
    });
  }
...
class DockToggleButton {
  ...
  update(newProps) {
    ...
    this.iconElement.className =
      'icon ' + getIconName(this.props.location, this.props.dockIsVisible);
  }
